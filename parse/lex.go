...
func lexEOSorSep(l *lexer) stateFn {
	if !isEOSorSep(l.peek()) {
		return lexStmt
	}
	for isEOSorSep(l.peek()) {
		if isEOS(l.peek()) {
			return lexEOS
		}
		l.next()
...
func isEOSorSep(r rune) bool {
	return isSep(r) || isEOS(r)
}
